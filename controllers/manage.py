...
def student_presentations():
    ...
    # don't allow deleting as the referencing will 
    # drop referenced rows from marking tables
    form = SQLFORM.grid(db.student_presentations, 
                        csv=False,
                        deletable=False)
...
def presentations():
    ...
    # don't allow deleting as the referencing to project marks will 
    # drop referenced rows from marking tables
    form = SQLFORM.grid(db.course_presentations, 
                        csv=False,
                        deletable=False)
...
def load_students():
    ...
    active_presentations = db().select(db.courses.id,
                                       db.courses.abbrname,
                                       db.course_presentations.id,
                                       db.course_presentations.name,
                                       db.course_presentations.is_active,
                                       orderby=[db.courses.id],
                                       left=db.course_presentations.on(db.courses.id == db.course_presentations.course)
                                       )
    
    # id maps
    active_pres_map = {ky: [v.course_presentations.id for v in vl if v.course_presentations.is_active]
    ...
    if form.accepts(request.vars):
        ...
        if len(bad_names) > 0:
            html = CAT(html,
                       ...
                       P('The student names data contains empty strings or '
