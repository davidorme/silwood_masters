...
def submitted_files():
    ...
    form = SQLFORM.grid(
        (db.marking_files.student == db.student_presentations.id)
...
def marking_files():
    ...
    file_missing_pres = db((db.marking_files.student == None)).select(
        db.marking_files.relative_url, db.marking_files.matching_issues
    )
    ...
    if len(pres_missing_file):
        ...
        pres_missing_file += TABLE(
            *[list(m.values()) for m in missing_rows],
    ...
    return dict(
        files_by_presentation=files_by_presentation,
...
def new_assignment():
    ...
    # Form requires an member of staff from the db and only allows assignments to be
    # added to student presentations from the current project year.
    db.assignments.marker.requires = IS_IN_DB(
        db, "teaching_staff.id", "%(last_name)s, %(first_name)s (%(email)s)"
    )

    ps_reqr = IS_IN_DB(
        db(db.student_presentations.academic_year == CURRENT_PROJECT_YEAR),
...
def assignments():
    ...
    if "all" in request.vars.keys():
        old_new = DIV(
            "This table is currently showing all marking assignments across all "
            "years. Click ",
            A("here", _href=URL("marking", "assignments")),
        ...
        old_new = DIV(
            "This table is currently only showing assignments for current students. "
    ...
    db.assignments.status.represent = lambda id, row: status_dict[
        row.assignments.status
    ]
    ...
    links = [
        dict(
            ...
            body=lambda row: A(
                ...
                _href=URL(
                    "marking", "write_report", vars={"record": row.assignments.id}
                ),
            ),
        ),
        dict(
            ...
            body=lambda row: A(
                "Edit",
                _class="button btn btn-secondary",
                _href=URL("marking", "edit_assignment", args=row.assignments.id),
            ),
        ),
    ]
    ...
    button_actions = (
        ("send", "Send to markers", distribute, {}),
    ...
    if actions.process().accepted:
        ...
        _, _, action_func, action_args = [
            b for b in button_actions if b[0] in request.post_vars
        ][0]
...
def marker_progress():
    ...
    # Build the table
    hdr = TR(
        *[TH(""), *[TH(x, _colspan=2, _width="20%") for x in count_template.keys()]]
    )
    subhdr = TR(
        [TH("")]
        + [
            ...
            TH(
                SPAN(
                    "",
                    _class="fa fa-check-circle",
                    _style="color:green;font-size: 1.3em;",
                )
            ),
        ]
        * len(count_template.keys())
    )
    table = [hdr, subhdr]
    ...
    for marker, data in groupby(status_count, lambda x: x.assignments.marker):
        ...
        for this_count in data:
            ...
            marker_counts[details.marker_role_id][
                details.status in ["Submitted", "Released"]
            ] += this_count.n

        # Style the row
        row = [
            [
                TD(B(v[0]), _style="background-color:salmon")
                if v[0]
                else TD(v[0], _style="color:black"),
...
def edit_assignment():
    ...
    form = SQLFORM(
        db.assignments, deletable=delete_ok, record=record, readonly=readonly
    )
...
def load_assignments():
    ...
    known_presentations = db(
        db.course_presentations.is_active == True,
    ).select(db.course_presentations.id, db.course_presentations.name)
    known_presentations = {rw.name: rw.id for rw in known_presentations}

    known_roles = db(db.marking_roles).select(
        db.marking_roles.id, db.marking_roles.name
    )
    ...
    form = SQLFORM.factory(
        ...
        Field("role_map", "text", default=default_rolemap, notnull=True),
        Field("data", "blob"),
    )
    ...
    if form.process(onvalidation=validate_assignments).accepted:
        ...
        unknown_presentations = set(fields["course_presentation"]).difference(
            known_presentations
        )
        ...
        if len(unknown_presentations) > 0:
            html = CAT(
                ...
                P(
                    "These course presentations are found in the file but are not recognized:"
                ),
        ...
        for this_student in data:
            student_record = (
                db(
                    (db.students.student_cid == int(this_student["student_cid"]))
                    & (
                        db.students.student_first_name
                        == this_student["student_first_name"]
                    )
                    & (
                        db.students.student_last_name
                        == this_student["student_last_name"]
                    )
                )
                .select()
                .first()
            ...
            stpres_record = (
                db(
                    (db.student_presentations.student == student_record.id)
                    & (
                        db.student_presentations.academic_year
                        == int(this_student["academic_year"])
                    )
                    & (
                        db.student_presentations.course_presentation
        ...
        if len(bad_student_records) > 0:
            html = CAT(
                ...
                P(
                    "The student data includes records with unknown CID and name combinations"
                ),
                P(
                    _format_error(
                        [
                            f"{st['student_cid']}' ({st['student_first_name']} "
        ...
        if len(bad_stpres_records) > 0:
            html = CAT(
                ...
                P(
                    _format_error(
                        [
                            f"{st['student_first_name']} {st['student_last_name']} "
        ...
        for this_staff in staff:
            staff_record = (
                db(db.teaching_staff.email.lower() == this_staff.lower())
                .select()
                .first()
            )
        ...
        if html != "":
            html = CAT(
                H2("Errors found in the uploaded file", _style="color:darkred"),
                P(
                    "Please edit the file to fix these problems and then repeat the upload"
                ),
                html,
            )
        else:
            ...
            for this_st in data:
                for this_mk in marker_headers:
                    assignments.extend(
                        [
                            {
                                "student_presentation": this_st["stpres_id"],
...
def validate_assignments(form):
    ...
    if not isinstance(role_map, dict):
        ...
        for vals in role_map_values:
            ...
            for idx, vl in enumerate(vals):
                if (
                    not isinstance(vl, dict)
                    or len(vl) != 2
                    or set(vl.keys()) != set(["Role", "Due"])
                ):
                    role_map_errors = True
                else:
                    role = vl["Role"]
    ...
    if role_map_errors:
        form.errors.role_map = (
            "The role map does not have the right structure: it should list "
...
def my_marking():
    ...
    if "all" in request.vars.keys():
        header = P(
            "The table below shows all the assignments that have been assigned to you ",
            B("across all years"),
            ". To focus on your current marking assignments, click ",
            A("here", _href=URL()),
        )
    else:
        qry &= db.student_presentations.academic_year == CURRENT_PROJECT_YEAR
        ...
        header = P(
            "The table below shows your ",
            B("current marking assignments"),
    ...
    return dict(
        name=marker.first_name + " " + marker.last_name, header=header, form=grid
    )
...
def presentation_overview():
    ...
    if rows:
        ...
        for idx, this_row in enumerate(rows):
            ...
            if this_row.status in ["Submitted", "Released"]:
                ...
                grades = [
                    P(f"{nm}: {gr}") for nm, gr in zip(display_grades, display_vals)
                ]
                grades = DIV(*grades)
                ...
                link = A(
                    "Link", _href=URL("write_report", vars=link_vars), _target="_blank"
                )
            else:
    ...
    hdr_rows = [
        TR(
            TD("Student:"),
            TD(f"{student.student_first_name} {student.student_last_name}"),
        ),
...
def write_report():
    ...
    if auth.has_membership("admin"):
        admin_warn = DIV(
            B("Reminder: "),
            "Logged in administrators have the ability to ",
    ...
    header = get_form_header(
        record, readonly, security=marker, show_due_date=show_due_date
    )
    ...
    if form.process(onvalidation=submit_validation).accepted:
        ...
        if "save" in list(request.vars.keys()):
            ...
            record.update_record(assignment_data=data, status="Started")
    ...
    if expected_files:
        ...
        if file_rows:
            files = CAT(
                H4("Files"),
                P(
                    "The following submitted files are available. These files are "
                ...
                UL(
                    [A(f.filename, _href=sharepoint.download_url(f)) for f in file_rows]
                ),
            )
        else:
            files = CAT(
                H4("Files"),
                P(
                    "The expected submitted files associated with this report "
    ...
    if not readonly:
        save_and_submit = DIV(
            ...
            " your progress and ",
            B("submit"),
            " it when it is complete, using the buttons at the bottom "
...
def criteria_and_forms():
    ...
    crit.represent = lambda value, rw: A(
        value, _href=URL("static", os.path.join("marking_criteria", value))
    )
...
def show_form():
    ...
    db.assignments._common_filter = None
    marker = db.teaching_staff.insert(
        first_name="Sir Alfred", last_name="Marker", email="null@null"
    )
    student = db.students.insert(
        student_first_name="Awesome",
        student_last_name="Student",
        student_email="null@null",
        student_cid="00000000",
        course=3,
    )
    student_presentation = db.student_presentations.insert(
        student=student, academic_year=2020, course_presentation=1
    )
    record = db.assignments.insert(
...
def download_pdf():
    ...
    if auth.has_membership("admin"):
        ...
        if (
            record.student_presentation.student.student_access_token
            != security["student_access_token"]
        ):
            session.flash = "Student access token invalid for PDF download"
            redirect(URL("index"))
        elif record.status != "Released":
            session.flash = (
                "Report not yet released. Also, we'd just love to know "
