...
class Mail:
    def __init__(self):
        ...
        self.use_imap = current.configuration.get("email.use_imap")
    ...
    def sendmail(
        ...
        if not self.logged_in:
            self._logmail(
                False,
                "mail send error - server logins failed",
                to=to,
                subject=subject,
        ...
        if text is not None:
            ...
            html_body = current.response.render(
                "email_templates/" + email_template, email_template_dict
            )
        ...
        try:
            ...
            self._logmail(
                False,
                "Message did not send (SMTP error)",
                to=to,
                subject=subject,
                email_template=email_template,
                email_template_dict=email_template_dict,
        ...
        # If IMAP is being used, save the message to the Marking_Reports folder.
        if self.use_imap:
            try:
                ...
                self._logmail(
                    False,
                    "Message did not store (IMAP error) - it _did_ send",
                    to=to,
                    subject=subject,
                    email_template=email_template,
                    email_template_dict=email_template_dict,
